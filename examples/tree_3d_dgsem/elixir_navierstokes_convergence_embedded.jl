...
tspan = (0.0, 0.5) # Same tspan as what 2D generated
ode = semidiscretize(semi, tspan)

# Read the file as an array of strings
lines = readlines("examples/tree_3d_dgsem/eigenvalues_NVS_convergence_tree_2d_dgsem.txt")

# Parse each line into ComplexF64
eig_vals_vector = ComplexF64.(parse.(Complex{Float64}, lines))

ode_algorithm = Trixi.EmbeddedPairedExplicitRK3(10, 10, tspan, eig_vals_vector)
...
time_int_tol = 1e-8
sol = Trixi.solve(ode, ode_algorithm,dt = 1.0, # Manual time step value, will be overwritten by the stepsize_callback when it is specified.
