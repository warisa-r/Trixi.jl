...
mutable struct LinearizedEulerEquations1D{RealT<:Real} <: LinearizedEulerEquations{1, 3}
  ...
  # Variables for computation of analytical solution
  eigenvalues::Vector{RealT}
  eigenvectors::Matrix{RealT}
  eigenvectors_inv::Matrix{RealT}
  ...
  function LinearizedEulerEquations1D(rho_0, v_0, c_0)
    ...
    # Known eigendecomposition
    newLinEuler1D.eigenvalues      = [v_0 - c_0; v_0; v_0 + c_0]
    newLinEuler1D.eigenvectors     = [-rho_0/c_0 1 rho_0/c_0; 1 0 1; -rho_0*c_0 0 rho_0*c_0]
    newLinEuler1D.eigenvectors_inv = inv(newLinEuler1D.eigenvectors)
...
@inline have_constant_speed(::LinearizedEulerEquations1D) = Val(true)
...
# Trace back characteristics. See https://metaphor.ethz.ch/x/2019/hs/401-4671-00L/literature/mishra_hyperbolic_pdes.pdf p.95
function compute_char_initial_pos(x, t, equations::LinearizedEulerEquations1D)
  return SVector(x[1], x[1], x[1]) .- t * equations.eigenvalues
end

function compute_primal_sol(char_vars, equations::LinearizedEulerEquations1D)
  return equations.eigenvectors * char_vars
end


# Follows from 2D case
